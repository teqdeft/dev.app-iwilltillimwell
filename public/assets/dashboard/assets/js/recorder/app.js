...
// 		document.getElementById("formats").innerHTML="Format: 1 channel pcm @ "+audioContext.sampleRate/1000+"kHz"
		...
		*/
	    ...
	    // Customizing error message for device not found
...
function pauseRecording(){
	...
	if (rec.recording){
            ...
            setTimeout( function() {
                ...
                recordButton.disabled = true;
                $(".pauseButton_play").show();
                $(".pauseButton_pause").hide();
            }, 900);
        ...
        $(".pauseButton_play").hide();
        $(".pauseButton_pause").show();
...
function stopRecording() {
    $(".pauseButton_play").hide();
    $(".pauseButton_pause").show();
    $("#voiceRecModalMsg").css("display","flex");
    
    if (recording_status) {

        
        $("#voiceRecModalMsg .message").html("<p>Recording Stop.Please wait...</p>");
        $("#voiceRecModalMsg .voiceRecModalMsgClose").hide();
        ...
        $("#voiceRecModalMsg .message").html("<p>Plase Start Recording First.</p>");
        $("#voiceRecModalMsg .voiceRecModalMsgClose").show();
    }
...
// 	li.appendChild(link);
    ...
    if (content.length > 6 && !hasConsecutiveDots) {
        ...
        instructions.text('Voice recognition stopped. Wait For saving....').css("color", "green");
        ...
        fetch('/upload-audio', {
            ...
            if (data.success) {
                ...
                deleteRec.setAttribute('data-recording-id', recordingId);
        ...
        output.val("");
        //voiceRecModal.modal('hide').data('bs.modal', null);
        $("#voiceRecModalMsg .message").html("<p>Not received any input. Please try again</p>");
        $("#voiceRecModalMsg .voiceRecModalMsgClose").show();
