<?php 
if (!in_array(request('action'), ['dermatology', 'urgentcare']) && Request::segment(3) == 'step-7') {
?>
@php
...
?>
        ...
        <a href="{{$scheduleUrl}}" class="outline-button"><i class="fa fa-chevron-left fa-arrow-icon fa-arrow-icon-back"></i> Back</a>
         ...
         <button type="button" class="btn btn-primary mr-3" type="button" onclick="return healthrisksubmit()">Next <i class="fa fa-chevron-right fa-arrow-icon"></i> </button>
...
function healthrisksubmit() {
    ...
    $('.input-list').each(function () {
        ...
        if (!hasChecked) {
            toastr.error("Please complete the questionnaire form.");

            const errorTarget = $(this).closest('.question-list');	
            errorTarget.addClass('error-title');
			
			
            // Delay scroll to ensure DOM updates apply
            setTimeout(function () {
                $('html, body').animate({
                    scrollTop: errorTarget.offset().top - 200 // tweak as needed
                }, 500);
            }, 100);
    ...
    if (!hasError) {
        window.location.href = "{{ $next_url }}"; // or use your actual variable
    }
}
