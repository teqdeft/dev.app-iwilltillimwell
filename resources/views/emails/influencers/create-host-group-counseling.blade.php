      ...
      <div class="main-wrapper" style="width: 100%;max-width: 730px;background:#f6eff5;margin: 50px auto;border-radius: 5px;box-shadow: 0px 4px 10px 2px #0000001c;">
         <div class="inner-wrapper">
            ...
            <div class="main-body-content" style="padding: 15px 30px;">
               <div class="inner-main-body-content">
                  <div class="intro-text-box">
                     <h4><span>Dear {{ $data['name'] }},</span></h4>
                        <p style="line-height: 1.5;">Welcome to iWILL ‘til i’mWELL. As a affilate.
                     </p>
                     <div class="newsletter-box">
                        <h4> To get started video stream please follow the link below :</h4>
                        <h4> <a href="{{ $data['link'] }}" target="_blank" style="color: #994c8d;">{{ $data['link'] }}</a></h4>
                        @if( !empty($data['counselingTime']) )
                            @foreach ($data['counselingTime'] as $key => $value)
                              <h5> Time : {{ date('Y F d h:i A',strtotime($value['start']))}} To {{date('h:i A',strtotime($value['end']))}} </h5>
                            @endforeach
                        @endif

                     </div>
                     <p style="line-height: 1.5;">
                       If you have any questions, please email Customer Service day or night at
                       <a href = "mailto: support@iwilltilimwell.com" style="color: #994c8d;
            ...
            <div class="footer" style="    background: #994c8d;color: #fff;padding: 24px;text-align: center;">
               <h3 style="margin-bottom:35px;text-align:center;font-weight:500;">Have Questions? Email Customer Service 24/7 at
                 <a href = "mailto: support@iwilltilimwell.com" style="color:#fff;">support@iwilltilimwell.com</a></h3>
                 <p style="line-height: 1.5;font-size:12px;text-align: center;margin-bottom:25px;">
                   © <?= date('Y') ?> iWILL ‘til i’mWELL. All rights reserved.
