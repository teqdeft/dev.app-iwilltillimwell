            ...
            if ($request->wantsJson() || $request->ajax()) {
                ...
                $promo->each(function ($item, $key) use ($jsonCollection) {
                    ...
                    $jsonCollection->push([
                        ...
                        'id' => $item->id,
                        'code' => $item->code,
                        'members_discount_amount' => ($item->member_discount_type == 'percentage')?round($item->member_discount_amount,2).'%':'$'.round($item->member_discount_amount,2),
                        'inc_name' => $user->name,
                        'influencer_discount_amount' => ($item->influencer_commission_type == 'percentage')?round($item->influencer_commission_amount,2).'%':'$'.round($item->influencer_commission_amount,2),
                        'influencer_payable_amount' => $influencer_payable_amount ? '$'. round($influencer_payable_amount,2) : "N/5A",
                        'allowed_members' => $item->allowed_members,
                        'status' => $item->status,
                    ]);
                });
    ...
    public function store(Request $request) {
        ...
        try {
            ...
            $data = array(
                ...
                'influencer_type' => $input['influencerType']
            );
